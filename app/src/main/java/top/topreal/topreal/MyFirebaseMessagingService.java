...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        wl = pm.newWakeLock(PowerManager., "My Tag");
        wl.acquire();*/
        //wl.release();
        ...
        if (remoteMessage.getNotification() != null) {
            ...
            try{
                JSONObject json_object = new JSONObject(notification_body);
                String contact = json_object.getString("contact");
                String session = json_object.getString("session");

                if (MainActivity.getInstace() == null){
                    throw new JSONException("MainActivity is null");
                }

                MainActivity.getInstace().setCurrentSessionID(session);

                switch (remoteMessage.getNotification().getTitle()) {
                    case "call_number":
                        MainActivity.getInstace().callPhoneNumber(contact);
                    break;
                    case "send_sms":
                        String sms_text = json_object.getString("sms_text");
                        MainActivity.getInstace().sendSMS(contact, sms_text);
                    break;
                }
            }
